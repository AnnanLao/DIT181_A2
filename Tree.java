...
class Tree<Item extends Comparable<Item>> {
	 ...
	 * removed is still a binary search tree. Wrapper method
	 * 
	 * @param i
	 *            - the item to remove
	 */
	 ...
	 * tree is BST. The auxiliary method
	 * 
	 * @param root
	...
	public void insertBST(Item item) {
		...
		this.root = insertBST(root, item);
	}

	public Node<Item> insertBST(Node<Item> node, Item item) {
		if (item == null) {
			return null;
		}
		if (node == null) {
	...
	public static void main(String[] args) {
		...
		t.printTree();
		System.out.println("nth Item bfs: " + t.nthBFS(8));
		System.out.println("dfs preorder toString: " + t.DFSToString());
		Tree<Integer> t1 = exampleTreeBin();
		t1.insertBST(6);
		t1.printTree();
	}
}
